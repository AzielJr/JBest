  ...
  const placeBets = async () => {
    ...
    try {
      ...
      for (const slip of betSlips) {
        ...
        if (!response.success) {
          throw new Error(response.message || 'Erro ao realizar aposta');
        }
      }
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm">
            ...
            <div className="mb-4">
              <div className="flex items-center text-sm text-gray-600 dark:text-gray-400 mb-2">
                ...
                Selecione {modalityConfig?.maxNumbers || 0} números de {modalityConfig?.numberRange[0].toString().padStart(2, '0') || '00'} a {modalityConfig?.numberRange[1].toString().padStart(2, '0') || '00'}
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm">
            ...
            <div className="flex items-center space-x-4">
              <button
                ...
                className="w-10 h-10 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors flex items-center justify-center"
              >
                <Minus size={16} />
              </button>
              ...
              <div className="flex-1">
                <input
                  ...
                  step="1"
                  className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-center font-medium"
                />
            ...
            <div className="mt-4 text-sm text-gray-600 dark:text-gray-400">
              Valor mínimo: {formatCurrency(modalityConfig.minBet)} | 
              Valor máximo: {formatCurrency(modalityConfig.maxBet)}
            </div>
            
            <button
              onClick={addToBetSlip}
              disabled={!modalityConfig || selectedNumbers.length !== modalityConfig.maxNumbers}
              className="w-full mt-4 bg-blue-600 text-white py-3 rounded-lg hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors font-medium"
            >
