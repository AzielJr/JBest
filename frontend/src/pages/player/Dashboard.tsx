...
const Dashboard: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Load recent bets
      const betsResponse = await bettingService.getBetHistory({ limit: 5 });
      if (betsResponse.success && betsResponse.data) {
        setRecentBets(betsResponse.data.bets);
      }

      // Load recent transactions
      const transactionsResponse = await walletService.getTransactionHistory({ limit: 5 });
      if (transactionsResponse.success && transactionsResponse.data) {
        setRecentTransactions(transactionsResponse.data.transactions);
      }

      // Load upcoming draws
      const drawsResponse = await bettingService.getCurrentDraws();
      if (drawsResponse.success && drawsResponse.data) {
        setUpcomingDraws(drawsResponse.data.slice(0, 3));
      }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-lg p-6 text-white">
        ...
        <p className="text-blue-100">
          Acompanhe suas apostas e gerencie sua carteira
        </p>
      </div>
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-gray-900 dark:text-white capitalize">
                      {transaction.type === 'deposit' ? 'Depósito' :
                       ...
                       transaction.type === 'bet' ? 'Aposta' : 'Prêmio'}
                    </p>
                    <p className="text-sm text-gray-600 dark:text-gray-400">
